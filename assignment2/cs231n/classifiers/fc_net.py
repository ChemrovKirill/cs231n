...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers):
            keys = [f'W{layer+1}', f'b{layer+1}', f'gamma{layer+1}', f'beta{layer+1}']
            ...
            if layer < self.num_layers - 1:
                if bn is not None:
                    if 'mode' in bn:
                        out, bn_cache = batchnorm_forward(out, gamma, beta, bn)
                    else:
                        out, ln_cache = layernorm_forward(out, gamma, beta, bn)
            ...
            cache[layer] = fc_cache, bn_cache, ln_cache, relu_cache, dropout_cache
            X = out
        ...
        for layer in range(self.num_layers-1, -1, -1):
            ...
            # caches from the forward pass
            fc_cache, bn_cache, ln_cache, relu_cache, dropout_cache = cache[layer]
            ...
            if dgamma is not None and layer < self.num_layers - 1:
