...
def affine_backward(dout, cache):
    ...
    db = np.sum(dout, axis=0)  # np.ones(N) @ dout
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        mask = (np.random.random(x.shape) < p).astype(int) / p
        out = x * mask
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    HO = 1 + (HI + 2*P - HF) // S    # output height      
    WO = 1 + (WI + 2*P - WF) // S    # output width

    w_row = w.reshape(F, -1)
    x_pad = np.pad(x, ((0,0), (0,0), (P, P), (P, P)), 'constant')
    x_fields = np.zeros((N, C, HF, WF, HO, WO))
    for hi in range(HO):
        for wi in range(WO):
            x_fields[..., hi, wi] = x_pad[...,hi*S:hi*S+HF, wi*S:wi*S+WF].copy()

    x_col = x_fields.reshape(N, C*HF*WF, -1)
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            ...
            for j in range(0, HO):
                for i in range(0, WO):
                    dw[f] += x_pad[n, :, j * S:j * S + HF, i * S:i * S + WF] * dout[n, f, j, i]
                    dx_pad[n, :, j * S:j * S + HF, i * S:i * S + WF] += w[f] * dout[n, f, j, i]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for j in range(HO):
            for i in range(WO):
                out[n, :, j, i] = np.amax(x[n, :, j*S:j*S+HF, i*S:i*S+WF], axis=(-1, -2))
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for j in range(HO):
                for i in range(WO):
                    ind = np.argmax(x[n, c, j*S:j*S+HF, i*S:i*S+WF])
                    ind1, ind2 = np.unravel_index(ind, (HF, WF))
                    dx[n, c, j*S:j*S+HF, i*S:i*S+WF][ind1, ind2] = dout[n, c, j, i]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape                                     
    x = np.moveaxis(x, 1, -1)       # N x H x W x C
    x = x.reshape(-1, C)            # N*H*W x C
    out, cache = batchnorm_forward(x, gamma, beta, bn_param)
    out = out.reshape(N, H, W, C)   # N x H x W x C
    out = np.moveaxis(out, -1, 1)   # N x C x H x W
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x = x.reshape(N*G, -1)  # N*G x C/G*H*W
...
def spatial_groupnorm_backward(dout, cache):
    ...
    G, cache = cache
    N, C, H, W = dout.shape
    x_norm = cache[-1].reshape(N, C, H, W)
...
def svm_loss(x, y):
    ...
    N = x.shape[0]
    correct_class_score = np.expand_dims(x[range(N), y], -1)
    margins = np.maximum(0, x - correct_class_score + 1)
    margins[range(N), y] -= 1  # because shouldn't add (x[:, y] - x[:, y] + 1)
    loss = (np.sum(margins) / N)

    dx = (margins > 0).astype(int)   # (N x C)
...
def softmax_loss(x, y):
    ...
    x_shift = x - np.max(x)                  # for robustness
    x_shiftexp = np.exp(x_shift)                 
    sumexp = np.sum(x_shiftexp, axis=1, keepdims=True)
    softmax = x_shiftexp / sumexp
    loss = np.mean(-x_shift[range(N), y] + np.log(sumexp))
    loss = loss

    softmax[range(N), y] -= 1  # grad for -x_shift[range(N), y]
    dx = softmax / N
